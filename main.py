...
# class autoscale_mon_data(BaseModel):
#     body: Union[str, None] = Field(default=None, example="af766182-101f-48be-8c80-0027b72c51f6")
...
###############################################################autoscale_for_monitoring###############################
@app.post("/autoscale_mon")
def do_autoscale_mon(data:dict):
    #parsed_data = json.loads(data)
    instance_id = data['alerts'][0]['labels']['instanceId']
    try:
        autoscale_vm = AutoScaleVm(instance_id)
        response_instance = autoscale_vm.get_instance()
        network_array = []
        for key in response_instance["server"]["addresses"].keys():
            network_array.append(key)
        ...
        ip_address = response_autoscale['Network'][network_key][0]['addr']
##=========================================== Loadbalancer ======================================================
        response_autoscale = autoscale_vm.get_interface(instance_id)
        ha = Octavia(project_id=response_instance["server"]["tenant_id"],instance_id=instance_id)
        ha_get = ha.get_octavia()
        pool_ids = []
        pool_exsit=[]
